...
if(implementation=="rf"){
  ...
  system(paste("rm -r " , parameters$Directories$FolderResults,
               "/Local2", sep=""))
  ...
  cat("\n\n########################################################")
    ...
    base_dir <- paste0(parameters$Directories$FolderResults, "/Local")
  ...
  # base_dir   <- "/tmp/d-GnegativeGO/Tested"
  base_dir   <- parameters$Directories$
  
  # output_tar <- "/tmp/d-GnegativeGO/GnegativeGO-results-hpml.tar.gz"
  output_tar <- paste0(parameters$Directories$folderResults, "/",
                       parameters$DatasetInfo$Name, "-results-local.tar.gz")
